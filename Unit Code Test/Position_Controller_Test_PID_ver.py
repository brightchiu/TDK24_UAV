...
POS_CAM_WIDTH = 3.68                # X軸Sensor寬度(mm)
POS_CAM_HEIGHT = 2.76               # Y軸Sensor高度(mm)
POS_X_VIEW_FIELD = 62.2             # X軸視角(degree)
POS_Y_VIEW_FIELD = 48.8             # Y軸視角(degree)
POS_CAM_OFFSET = 55                 # 相機(鏡頭)至地板高度(mm)
...
def position_controller():
    ...
    while STATE_SYSTEM:
        while MODE == 'POS' and STATE_SYSTEM:
            ...
            # 轉換至平面距離座標 (px => mm)
            #pos_x = block_x * ((POS_CAM_WIDTH/ 2) * (1/math.tan(POS_X_VIEW_FIELD/2)) +
            #                   ACTUAL_ALT*1000 + POS_CAM_OFFSET) * math.tan(POS_X_VIEW_FIELD/2)

            #pos_y = block_y * ((POS_CAM_HEIGHT / 2) * (1 / math.tan(POS_Y_VIEW_FIELD / 2)) +
            #                   ACTUAL_ALT*1000 + POS_CAM_OFFSET) * math.tan(POS_Y_VIEW_FIELD / 2)
