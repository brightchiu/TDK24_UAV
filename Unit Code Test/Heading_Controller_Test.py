...
WEIGHTED_SONAR = 7              # 超音波高度計權重*
WEIGHTED_BAROM = 3              # 氣壓高度計權重*

# ->超音波高度計參數
SONAR_AIR_TEMP = 27             # 氣溫(攝氏)*
...
SONAR_FILTER_ALT = 0.4          # 突波濾波作用起始高度(m)
...
def detect_arrive(color):
    ...
    while img_area < IMG_AREA_LIMIT:
        ...
        print('<飛行狀態> 循跡中，差角%.2f度，角速度%.2f度/秒' % (ACTUAL_HDG_ERR, TARGET_YAW_RATE))
        time.sleep(1)

    # 回報偵測到顏色
    print('<飛行狀態> 抵達%s色塊上方' % color)
...
# 任務主函式
def main():
    global STATE_SYSTEM, MISSION_COLOR
    # 啟動前檢查，回報主程式啟動
    pre_check()
    print('<啟動程序> 主程序：啟動')
    ...
    take_off(1)                         # 起飛
    flight_forward()                    # 循線飛行
    detect_leave('black')               # 離開條件偵測
    detect_arrive('red')                # 抵達條件偵測
    MISSION_COLOR = 'red'               # 設定任務顏色旗標
    landing()                           # 執行降落
