...
# ========== 旗標與變數 ================================================
# 旗標
STATE_SYSTEM = False            # 系統旗標(關閉線程用)
STATE_INFO = False              # GUI資訊更新旗標
...
def img_calculate(color):
    ...
    # 取得輪廓點與索引 =====> 此用cv2版本二格式，只有兩個回傳值
    contours, hierarchy = cv2.findContours(image_mask, cv2.RETR_TREE,
...
def img_moment():
    ...
    # 取得輪廓點與索引 =====> 此用cv2版本二格式，只有兩個回傳值
    contours, hierarchy = cv2.findContours(img_th, cv2.RETR_TREE,
                                           cv2.CHAIN_APPROX_SIMPLE)
...
def info_update():
    ...
    while STATE_INFO:
        ...
        # 3-1 姿態更新
        info_roll.configure(text='%.2f' % math.degrees(uav.attitude.roll))
        info_pitch.configure(text='%.2f' % math.degrees(uav.attitude.pitch))
        info_yaw.configure(text='%d' % math.degrees(uav.attitude.yaw))
        ...
        # 4-1 高度控制器更新
        alt_target.configure(text=TARGET_ALT)
        ...
        # 5 影像更新
        if STATE_CAM:
            if MODE == 'POS' or MODE == 'LINE' and STATE_POS:
...
def thread_procedure1():
    global STATE_SYSTEM, STATE_INFO, STATE_THREAD
    # 啟動系統與資訊更新旗標
    STATE_SYSTEM = True
    STATE_INFO = True

    # 定義多線程
    iu = threading.Thread(target=info_update)       # 參數更新線程啟動
...
# ========== 數值紀錄區塊 ================================================
def show_data():
    messagebox.showinfo('<關閉程序>', '將關閉視窗以顯示飛行資料圖表')
    flight_monitor.destroy()
    # ======高度控制器================
    # 高度控制器響應圖
    num_alt = []
...
# 1 標題區
row_1_frame = tk.Frame(flight_monitor, height=80, width=1280, bg='#eff8fd')
...
# 2 系統狀態列
row_2_frame = tk.Frame(flight_monitor, height=150, width=1280, bg='#eff8fd')
...
# 系統控制器
state_sys_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 模式控制器
state_mode_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 線程控制器
state_thr_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 高度控制器
state_alt_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 位置控制器
state_pos_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 航向控制器
state_hdg_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 命令控制器
state_cmd_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 相機控制器
state_cam_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 高度感測器
state_alsen_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 超音波感測器
state_sonar_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 沙包盒艙門
state_box_frame = tk.Frame(state_frame, height=130, width=115, bg='#eff8fd')
...
# 3 即時飛行資料
row_3_frame = tk.Frame(flight_monitor, height=150, width=1280, bg='#eff8fd')
...
# 滾轉
info_roll_frame = tk.Frame(info_frame, height=130, width=100, bg='#eff8fd')
...
# 俯仰
info_pitch_frame = tk.Frame(info_frame, height=130, width=100, bg='#eff8fd')
...
# 偏航
info_yaw_frame = tk.Frame(info_frame, height=130, width=100, bg='#eff8fd')
...
# 航向
info_hdg_frame = tk.Frame(info_frame, height=130, width=100, bg='#eff8fd')
...
# 高度
info_alt_frame = tk.Frame(info_frame, height=130, width=100, bg='#eff8fd')
...
# 超音波高度計
info_alt_sonar_frame = tk.Frame(info_frame, height=130, width=100, bg='#eff8fd')
...
# 氣壓高度計
info_alt_barom_frame = tk.Frame(info_frame, height=130, width=100, bg='#eff8fd')
...
button_thread = tk.Button(button_thread_frame, font=('', 13),
                          text='啟 動', width=5, height=2, command=thread_procedure1, bg='#eff8fd')
...
button_mission = tk.Button(button_mission_frame, font=('', 13),
                           text='啟 動', width=5, height=2, command=thread_procedure2, bg='#eff8fd')
...
button_mission_label.pack(side=tk.TOP)
button_mission = tk.Button(button_mission_frame, font=('', 13),
                           text='取 得', width=5, height=2, command=show_data, bg='#eff8fd')
button_mission.pack(side=tk.TOP)

# 任務階段
info_mission_frame = tk.Frame(row_3_frame, height=150, width=250, bg='#eff8fd')
...
info_mission = tk.Label(info_mission_frame, font=('', 30), text='[飛行前檢查]', bg='#eff8fd', fg='red')
info_mission.pack(side=tk.TOP)


# 相機處理框
cam_frame = tk.Frame(flight_monitor, width=400, height=400, bg='#eff8fd')
...
# 4 控制器資訊
row_4_frame = tk.Frame(flight_monitor, height=150, width=1280, bg='#eff8fd')
...
# 目標高
alt_target_frame = tk.Frame(alt_frame, height=130, width=100, bg='#eff8fd')
...
# 實際高度
alt_actual_frame = tk.Frame(alt_frame, height=130, width=100, bg='#eff8fd')
...
# 輸出推力
alt_thrust_frame = tk.Frame(alt_frame, height=130, width=100, bg='#eff8fd')
...
# 任務顏色
pos_mis_color_frame = tk.Frame(pos_frame, height=130, width=100, bg='#eff8fd')
...
# 定位顏色
pos_color_frame = tk.Frame(pos_frame, height=130, width=100, bg='#eff8fd')
...
# 型心位置
pos_actual_frame = tk.Frame(pos_frame, height=130, width=170, bg='#eff8fd')
...
# 滾轉
pos_roll_frame = tk.Frame(pos_frame, height=130, width=100, bg='#eff8fd')
...
# 俯仰
pos_pitch_frame = tk.Frame(pos_frame, height=130, width=100, bg='#eff8fd')
...
# 5 控制器資訊
row_5_frame = tk.Frame(flight_monitor, height=150, width=1280, bg='#eff8fd')
...
# 角度差
hdg_error_frame = tk.Frame(hdg_frame, height=130, width=100, bg='#eff8fd')
...
# 角速度
hdg_yaw_rate_frame = tk.Frame(hdg_frame, height=130, width=100, bg='#eff8fd')
...
# 滾轉角
hdg_roll_frame = tk.Frame(hdg_frame, height=130, width=100, bg='#eff8fd')
...
msg_window = tk.Text(msg_frame, font=('', 15), width=60, height='5')
