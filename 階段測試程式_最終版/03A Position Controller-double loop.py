...
# 0920 修改位置控制器為雙迴圈型態，確保定位準確
# 0924 修正外迴圈狀態輸出，位置到速度
# 0926 於速度值計算增加均值濾波，防止因秒數造成的跳動
...
POS_KP = 0.02                   # 位置控制器-P增益值
POS_KI = 0                      # 位置控制器-I增益值
POS_KD = 0.02                   # 位置控制器-D增益值
POS_VEL_LIMIT = 20              # 位置控制器-速度輸出限制(px/s，1.2m高時1px=3.02mm，建議低於100 )
POS_VEL_KP = 0.025              # 速度控制器-P增益值
POS_VEL_KI = 0                  # 速度控制器-I增益值
POS_VEL_KD = 0                  # 速度控制器-D增益值
...
def position_controller():
    ...
    while STATE_THREAD:
        while MODE == 'POS' and STATE_THREAD:
            ...
            # X軸誤差增益處理
            pos_gain_p_x = pos_error_x * POS_KP
            pos_gain_i_x = pos_int_error_x * POS_KI
            pos_gain_d_x = pos_derivative_x * POS_KD
            vel_x_out = pos_gain_p_x + pos_gain_i_x + pos_gain_d_x

            # Y軸誤差增益處理
            pos_gain_p_y = pos_error_y * POS_KP
            pos_gain_i_y = pos_int_error_y * POS_KI
            pos_gain_d_y = pos_derivative_y * POS_KD
            ...
            # ->速度控制器區塊A-取值
            # 計算型心移動速度
            pos_delta_x = block_x - block_x_old
            pos_delta_y = block_y - block_y_old
            ...
            if len(vel_x_register) > POS_SAMPLE_NUM_AVE:
                vel_x_register.pop()
            vel_x_actual = np.average(vel_x_register)

            # Y速度均值濾波器
            vel_y_register.insert(0, vel_y_raw)
            if len(vel_y_register) > POS_SAMPLE_NUM_AVE:
                vel_y_register.pop()
            vel_y_actual = np.average(vel_y_register)

            # ->速度控制器區塊B-控制器
            # 計算誤差值
            vel_error_x = vel_x_cmd - vel_x_actual
            vel_error_y = vel_y_cmd - vel_y_actual
            ...
            # X軸誤差增益處理
            vel_gain_p_x = vel_error_x * POS_VEL_KP
            vel_gain_i_x = vel_int_error_x * POS_VEL_KI
            vel_gain_d_x = vel_derivative_x * POS_VEL_KD
            roll_out = vel_gain_p_x + vel_gain_i_x + vel_gain_d_x

            # Y軸誤差增益處理
            vel_gain_p_y = vel_error_y * POS_VEL_KP
            vel_gain_i_y = vel_int_error_y * POS_VEL_KI
            vel_gain_d_y = vel_derivative_y * POS_VEL_KD
            ...
            R_POS_VEL_X_CMD.append(vel_x_cmd)
            R_POS_VEL_Y_CMD.append(vel_y_cmd)
            R_POS_VEL_X_ACT.append(vel_x_actual)
            R_POS_VEL_Y_ACT.append(vel_y_actual)
        ...
        R_POS_VEL_X_CMD.append(0)
        R_POS_VEL_Y_CMD.append(0)
...
def pos_ctrl_data():
    ...
    plt.plot(num_pos, R_POS_VEL_X_CMD, label="X Velocity Output")
    ...
    plt.plot(num_pos, R_POS_VEL_Y_CMD, label="Y Velocity Output")
    plt.plot(num_pos, R_POS_BLOCK_AREA, label="Block Area")
    ...
    # 速度控制器X響應圖
    plt.plot(num_pos, R_POS_VEL_X_CMD, label="Target X Velocity")
    ...
    # 速度控制器Y響應圖
    plt.plot(num_pos, R_POS_VEL_Y_CMD, label="Target Y Velocity")
