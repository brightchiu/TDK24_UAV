...
POS_X_KP = 0.02                 # 位置控制器X軸-P增益值
POS_X_KI = 0                    # 位置控制器X軸-I增益值
POS_X_KD = 0.02                 # 位置控制器X軸-D增益值
POS_Y_KP = 0.02                 # 位置控制器Y軸-P增益值
POS_Y_KI = 0                    # 位置控制器Y軸-I增益值
POS_Y_KD = 0.02                 # 位置控制器Y軸-D增益值
POS_VEL_LIMIT = 1000            # 位置控制器-速度輸出限制(px/s，1.2m高時1px=3.02mm，建議低於100 )
...
POS_ANGLE_LIMIT = 5             # 速度控制器-最大飛行角度(degree)
POS_VEL_X_FEEDBACK_GAIN = 0.2   # X軸速度回授增益
POS_VEL_Y_FEEDBACK_GAIN = 0.2   # Y軸速度回授增益

# ->飛行程序參數
TAKEOFF_SETTLE = 95             # 安定高度百分比(%，高度確認門檻)*
LANDING_CUTOFF = 0.05           # 油門關閉高度(m，停止油門輸出)*
LANDING_TARGET = -0.5           # 將路目標高度(負值防止落地震盪)
...
def camera_sensor():
    ...
    while STATE_THREAD:
        ...
        while MODE == 'POS' and STATE_THREAD:
            ...
            # 出界數值保留機制
            if pos_x == 0 and math.fabs(pos_x_old) > BODY_REF_X/2:
                pos_x = pos_x_old
            if pos_y == 0 and math.fabs(pos_y_old) > BODY_REF_Y/2:
                pos_y = pos_y_old
...
def thread_activate():
    ...
    vel_ctrl = threading.Thread(target=velocity_controller)
...
def take_off(target_alt):
    ...
    # 等待抵達目標高度
    while ACTUAL_ALT < TARGET_ALT * (TAKEOFF_SETTLE / 100) and STATE_THREAD:
...
def landing():
    ...
    TARGET_ALT = LANDING_TARGET          # 負值以確保不會著地後彈升

    # 等待抵達目標高度
    while ACTUAL_ALT > LANDING_CUTOFF and STATE_THREAD:
        ...
        if ACTUAL_ALT_SONAR < LANDING_CUTOFF:
            break
        time.sleep(1)
