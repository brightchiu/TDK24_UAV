...
FREQ_SENSOR_POS = 20            # 位置感測器頻率(不快於影像禎率)
...
HDG_SAMPLE_NUM_AVE = 10         # 航向控制器均值濾波取樣次數*
...
SPD_KI = 0                      # I增益值*
SPD_KD = 0                      # D增益值*
SPD_ANGLE_LIMIT = 2             # 飛行速度控制器-最大俯仰角度(degree)
SPD_LIMIT = 0.5                 # 飛行速度控制器-最大飛行速度(m/s)
SPD_SAMPLE_NUM_AVE = 5          # 飛行速度控制器均值濾波取樣次數*
...
# ->狀態記錄
R_UAV_VEL_X = []                        # X方向速度(北方)
R_UAV_VEL_Y = []                        # Y方向速度(東方)
R_UAV_VEL_Z = []                        # Z方向速度(下方)
R_UAV_ROLL = []                         # 滾轉角(右正左負)
R_UAV_PITCH = []                        # 俯仰角(上正下負)
R_UAV_YAW = []                          # 偏航角(右正左負)
...
def altitude_controller():
    ...
    while STATE_THREAD:
        ...
        height_out = gain_p + gain_i + gain_d

        # 變更至速度狀態
        speed_out = height_out / delta_time
...
def position_controller():
    ...
    while STATE_THREAD:
        while MODE == 'POS' and STATE_THREAD:
            ...
            # 回授值截止區修正
            if -POS_DZ < pos_actual_x < POS_DZ:
                pos_actual_x = 0
            if -POS_DZ < pos_actual_y < POS_DZ:
                pos_actual_y = 0

            # 計算誤差值
            pos_error_x = -pos_actual_x
            pos_error_y = -pos_actual_y
            ...
            # 轉換成速度命令(取消狀態更改，改成直接輸出)
            # vel_x_out /= delta_time
            # vel_y_out /= delta_time
...
def heading_controller():
    global TARGET_YAW_RATE, STATE_HDG
    # PID變數設定
    int_error = 0
    previous_error = 0
    previous_time = time.time()
...
# ========== 影像感測器 ===============================================
# 負責計算影像的位置、速度、角度數值，結合位置與循線模式
def camera_sensor():
    global STATE_SENSOR_CAM, ACTUAL_POS, ACTUAL_VEL, ACTUAL_AREA, ACTUAL_HDG
    # 均值濾波器設定
    pos_x_register = []
    pos_y_register = []
    vel_x_register = []
    ...
    while STATE_THREAD:
        ...
        while MODE == 'POS' and STATE_THREAD:
            ...
            if len(vel_x_register) > POS_SAMPLE_NUM_AVE:
                vel_x_register.pop()
            vel_x = np.average(vel_x_register)

            # Y速度均值濾波器
            vel_y_register.insert(0, vel_y_raw)
            if len(vel_y_register) > POS_SAMPLE_NUM_AVE:
                vel_y_register.pop()
        ...
        while MODE == 'LINE' and STATE_THREAD:
            # 取得線條型心位置
            pos_x_raw, pos_y_raw = line_moment()

            # 型心位置均值濾波器
            # X座標均值濾波器
            pos_x_register.insert(0, pos_x_raw)
            if len(pos_x_register) > HDG_SAMPLE_NUM_AVE:
                pos_x_register.pop()
            pos_x = np.average(pos_x_register)

            # Y座標均值濾波器
            pos_y_register.insert(0, pos_y_raw)
            if len(pos_y_register) > HDG_SAMPLE_NUM_AVE:
                pos_y_register.pop()
            pos_y = np.average(pos_y_register)
...
def line_moment():
    ...
    # 遮罩流程
    mask = np.zeros_like(img_th)    # 製作遮罩，長寬和binary相同
    ignore_mask_color = 255         # 覆蓋掉的顏色，不用改
    img_shape = CAM_IMAGE.shape
    # 做水平和垂直線各三條，用來切分出九塊區域
    block = np.array([[(img_shape[1] * 0, img_shape[0] * 0.25),
                       (img_shape[1] * 1, img_shape[0] * 0.25),
                       (img_shape[1] * 1, img_shape[0] * 0),
                       (img_shape[1] * 0, img_shape[0] * 0)]], dtype=np.int32)

    # 將遮罩和九條線疊合
    cv2.fillPoly(mask, block, ignore_mask_color)

    # 將二值化後的影像和遮罩疊合
    img_masked = cv2.bitwise_and(img_th, mask)

    # 輸出影像(GUI顯示用)
    if MODE == 'LINE':
        CAM_IMAGE_LIVE = img_masked

    # 取得影像、輪廓點與索引
    image_c, contours, hierarchy = cv2.findContours(img_masked, cv2.RETR_TREE,
                                                    cv2.CHAIN_APPROX_SIMPLE)
...
def data_sheet():
    ...
    ds3.cell(2, 2, '輸出偏航角速率')
    for x in range(len(R_HDG_TARGET_YAW_RATE)):
        ds3.cell(x + 3, 2, R_HDG_TARGET_YAW_RATE[x])

    # 航向控制器PID數值處理圖
    ds3.cell(1, 4, '航向控制器PID數值處理圖')
    ds3.cell(2, 4, 'Error')
    for x in range(len(R_HDG_ERROR)):
        ds3.cell(x + 3, 4, R_HDG_ERROR[x])

    ds3.cell(2, 5, 'Gain P')
    for x in range(len(R_HDG_GP)):
        ds3.cell(x + 3, 5, R_HDG_GP[x])

    ds3.cell(2, 6, 'Gain I')
    for x in range(len(R_HDG_GI)):
        ds3.cell(x + 3, 6, R_HDG_GI[x])

    ds3.cell(2, 7, 'Gain D')
    for x in range(len(R_HDG_GP)):
        ds3.cell(x + 3, 7, R_HDG_GP[x])

    # 飛行速度控制器
    ds3.cell(1, 9, '飛行速度控制器數值處理圖')
    ds3.cell(2, 9, '目標速度')
    for x in range(len(R_SPD_TARGET)):
        ds3.cell(x + 3, 9, R_SPD_TARGET[x])

    ds3.cell(2, 10, '實際速度')
    for x in range(len(R_SPD_ACTUAL)):
        ds3.cell(x + 3, 10, R_SPD_ACTUAL[x])

    ds3.cell(2, 11, '目標傾角')
    for x in range(len(R_SPD_TARGET_PITCH)):
        ds3.cell(x + 3, 11, R_SPD_TARGET_PITCH[x])

    ds3.cell(2, 12, '實際傾角')
    for x in range(len(R_SPD_ACTUAL_PITCH)):
        ds3.cell(x + 3, 12, R_SPD_ACTUAL_PITCH[x])

    # 飛行速度控制器PID數值處理圖
    ds3.cell(1, 14, '飛行速度控制器PID數值處理圖')
    ds3.cell(2, 14, 'Error')
    for x in range(len(R_SPD_ERROR)):
        ds3.cell(x + 3, 14, R_SPD_ERROR[x])

    ds3.cell(2, 15, 'Gain P')
    for x in range(len(R_SPD_GP)):
        ds3.cell(x + 3, 15, R_SPD_GP[x])

    ds3.cell(2, 16, 'Gain I')
    for x in range(len(R_SPD_GI)):
        ds3.cell(x + 3, 16, R_SPD_GI[x])

    ds3.cell(2, 17, 'Gain D')
    for x in range(len(R_SPD_GP)):
        ds3.cell(x + 3, 17, R_SPD_GP[x])
...
def vel_ctrl_data():
    ...
    # 速度控制器X響應圖
    plt.plot(num_pos, R_POS_VEL_X_TAR, label="Target X Velocity")
    plt.plot(num_pos, R_POS_VEL_X_ACT, label="Actual X Velocity")
    plt.plot(num_pos, R_POS_TARGET_ROLL, label="Target Roll Angle")
    plt.plot(num_pos, R_POS_ACTUAL_ROLL, label="Actual Roll Angle")
    ...
    # 速度控制器Y響應圖
    plt.plot(num_pos, R_POS_VEL_Y_TAR, label="Target Y Velocity")
    plt.plot(num_pos, R_POS_VEL_Y_ACT, label="Actual Y Velocity")
    plt.plot(num_pos, R_POS_TARGET_PITCH, label="Target Pitch Angle")
    plt.plot(num_pos, R_POS_ACTUAL_PITCH, label="Actual Pitch Angle")
    ...
    # 速度控制器X-PID響應圖
    plt.plot(num_pos, R_POS_VEL_X_ERROR, label="Error")
    plt.plot(num_pos, R_POS_VEL_X_GP, label="Gain P")
    plt.plot(num_pos, R_POS_VEL_X_GI, label="Gain I")
    plt.plot(num_pos, R_POS_VEL_X_GD, label="Gain D")
    ...
    # 速度控制器Y-PID響應圖
    plt.plot(num_pos, R_POS_VEL_Y_ERROR, label="Error")
    plt.plot(num_pos, R_POS_VEL_Y_GP, label="Gain P")
    plt.plot(num_pos, R_POS_VEL_Y_GI, label="Gain I")
    plt.plot(num_pos, R_POS_VEL_Y_GD, label="Gain D")
