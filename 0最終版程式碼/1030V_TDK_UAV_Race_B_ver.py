...
# ->循跡-滾轉控制器參數(比照定位X軸處理)
# 外迴圈
HDG_X_KP = 0.2                  # 位置控制器P增益值(回至中心線速度)*
...
# 內迴圈
HDG_VX_KP = 0.6                # 速度控制器P增益值(回至中心線速度)*
HDG_VX_KI = 0                   # 速度控制器I增益值(有穩差才使用)*
HDG_VX_KD = 0.3                # 速度控制器D增益值(減少震盪並拉快安定時間)*
HDG_ROLL_LIMIT = 5              # 速度控制器-最大飛行角度(degree)
HDG_ROLL_CUTOFF = 0           # 速度控制器-最小飛行角度(degree)
HDG_VX_KH = 1                   # X軸回授增益(與控制器增益有相依性)
...
# ->飛行程序參數
TAKEOFF_SETTLE = 55             # 安定高度百分比(%，高度確認門檻)*
...
# ->線角度計算
ANG_THRESHOLD = 30              # 二值化門檻值(調高比較能看到線，但可能有雜訊)*
