...
# 1101測試循線(面積與影像顯示測試、低空循線測試、顏色轉換模式更改)
# T路口轉彎邏輯確認(模測通過)、下降階段應對規劃
# 沙包盒需做修正包裝
...
# ->飛行程序參數
TAKEOFF_SETTLE = 80             # 安定高度百分比(%，高度確認門檻)*
...
# ->影像計算參數
IMG_BLUE_L = np.array([85, 76, 30])         # 藍色色域-下限
    ...
    for c in contours:
        ...
        if block_area >= ANG_LINE_TRACK_AREA_L:
            ...
            if block_area > ANG_LINE_TRACK_AREA_H:      # 面積大於限制時不輸出角度
                ...
                if ANG_T_TURN == 'left' or l_direction == 'left':        # 左T
                    line_angle *= -1
                elif ANG_T_TURN == 'right' or l_direction == 'right':     # 右T
                    line_angle *= 1
...
def l_corner_detect():
    ...
    if left_flag and down_flag:
        return 'left'
    elif right_flag and down_flag:
        return 'right'
    elif up_flag and down_flag:
        return 'straight'
    else:
        return 'NaN'
...
def t_intersection_detect():
    ...
    if up_flag and down_flag and left_flag:
        return 'left'
    elif up_flag and down_flag and right_flag:
        return 'right'
...
def wait_t_intersection():
    ...
    global ANG_T_TURN, TARGET_PITCH
    # 一般循線段
    i = 0
