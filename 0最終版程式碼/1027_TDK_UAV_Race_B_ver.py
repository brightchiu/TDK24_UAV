...
# 定位定高皆已通過驗證測試(穩定性90％)
# 1028執行循線校正專案(包含起飛區、轉彎區、T字路口區)
# 1029執行沙包、色塊抓取降落、燈號轉彎測試
...
TARGET_SPD = 0                  # 目標飛行速度
TARGET_LINE_POS = 0             # 線型心座標(px，相機座標)
...
ACTUAL_LINE_VEL = 0             # 線距離(px/s，Y軸為基準，Roll控制器回授用)
...
ACTUAL_VY = 0                   # Y方向速度
ACTUAL_VZ = 0                   # Z方向速度
ACTUAL_AX = 0                   # X方向加速度(m/s^2)
ACTUAL_AY = 0                   # Y方向加速度(m/s^2)
ACTUAL_AZ = 0                   # Z方向加速度(m/s^2)
...
VEL_X_DZ = 30                   # 輸入截止區參數(於截止區內使速度控制器輸出零角度，防震盪)
VEL_Y_DZ = 30                   # 輸入截止區參數(於截止區內使速度控制器輸出零角度，防震盪)
VEL_CUTOFF_ALT = 0.25           # 控制器最低作動高度(m，飽和高度點)
...
# ->循跡-滾轉控制器參數(比照定位X軸處理)
# 外迴圈
HDG_X_KP = 0.3                  # 位置控制器P增益值(回至中心線速度)*
HDG_X_KI = 0                    # 位置控制器I增益值(有穩差才使用)*
HDG_X_KD = 0                    # 位置控制器D增益值(減少震盪並拉快安定時間)*
HDG_X_DZ = 40                   # 位置控制器輸入截止區參數(於截止區內使速度控制器控制零速度，防震盪)
...
HDG_VX_DZ = 30                  # 輸入截止區參數(於截止區內使速度控制器輸出零角度，防震盪)
...
def heading_controller():
    ...
    previous_time_in = time.time()

    # 均值濾波設定
    vx_cmd_register = []

    # 內外迴圈差異進位數計算
    loop_carry_num = FREQ_HDG_IN / (FREQ_HDG_IN - FREQ_HDG_OUT)
    loop_num = 0
    ...
    while STATE_THREAD:
        ...
        while MODE == 'LINE' and STATE_THREAD:
            ...
            if loop_num % loop_carry_num == 0:
                ...
                # 誤差值微積分計算
                current_time_out = time.time()
                delta_time_out = current_time_out - previous_time_out

                delta_error_x = error_x - previous_error_x
                int_error_x += (error_x * delta_time_out)
                derivative_x = delta_error_x / delta_time_out
                ...
                # X速度命令均值濾波器
                vx_cmd_register.insert(0, vx_out)
                if len(vx_cmd_register) > HDG_SAMPLE_NUM_AVE:
                    vx_cmd_register.pop()
                vx_cmd = np.average(vx_cmd_register)

                # 傳遞值給下次迴圈使用
                previous_error_x = error_x
                previous_time_out = current_time_out

                # 記錄數值
                # 外迴圈
                R_HDG_LINE_ACTUAL_X.append(ACTUAL_LINE_POS)
                R_HDG_LINE_TARGET_VX.append(vx_cmd)
            ...
            # 回授值截止區修正
            if vx_cmd - VEL_X_DZ < actual_vx < vx_cmd + VEL_X_DZ:
                actual_vx = vx_cmd

            # 計算誤差值
            error_vx = vx_cmd - actual_vx
            error_hdg = TARGET_HDG - ACTUAL_HDG

            # 誤差值微積分計算
            current_time_in = time.time()
            delta_time_in = current_time_in - previous_time_in

            delta_error_vx = error_vx - previous_error_vx
            int_error_vx += (error_vx * delta_time_in)
            derivative_vx = delta_error_vx / delta_time_in

            delta_error_hdg = error_hdg - previous_error_hdg
            int_error_hdg += (error_hdg * delta_time_in)
            derivative_hdg = delta_error_hdg / delta_time_in
            ...
            previous_time_in = current_time_in
            loop_num += 1

            # 記錄數值
            # 內迴圈Roll
            R_HDG_LINE_VX_TARGET.append(vx_cmd)
...
def camera_sensor():
    ...
    global ACTUAL_HDG, ACTUAL_LINE_POS, ACTUAL_LINE_VEL
    # 均值濾波器設定
    pos_x_register = []
    pos_y_register = []
    vel_x_register = []
