...
# 問題處理：定高穩差、重心與前進速率控制(慢者為佳)
# 循線組測試：起飛T型區、L轉角、中間投送、T字路口
...
# ->循跡-滾轉控制器參數(比照定位X軸處理)
# 外迴圈
HDG_X_KP = 0.5                  # 位置控制器P增益值(回至中心線速度)*
...
SPD_PITCH_OVER_WRITE = True     # Pitch覆寫旗標
SPD_KP = 8                      # P增益值*
SPD_KI = 0                      # I增益值*
SPD_KD = 0                      # D增益值*
...
SPD_OFF_ANGLE = 10              # 切換跳出角度(degree)，慣性抵銷用
SPD_SAMPLE_NUM_AVE = 5          # 飛行速度感測器均值濾波取樣次數*

# ->飛行速度控制器參數-開迴路
SPD_OL_ACTIVE_TIME = 2          # 開迴路控制模式，角度作動時長(s)
SPD_OL_OFF_TIME = 5             # 開迴路控制模式，角度關閉時長(s)
SPD_OL_ANGLE = 3                # 開迴路控制模式，角度作動量(degree，由呼叫函式控制)

# ->飛行程序參數
TAKEOFF_SETTLE = 70             # 安定高度百分比(%，高度確認門檻)*
...
T_JUDGE_TIMES = 2               # T字路口判斷閥值(次數)
T_RED_ARRIVE_LIMIT = 1000       # T字路口離開條件(px，紅色線條面積)
...
# ->線角度計算
ANG_LINE_TRACK_AREA_L = 3000     # 線追蹤型心計算面積下限*
ANG_LINE_TRACK_AREA_H = 10000    # 線追蹤型心計算面積下限(僅限制角度輸出)
ANG_SAMPLE_NUM_AVE = 5          # 航向控制器均值濾波取樣次數*
ANG_T_INTERSECT_THRESHOLD_DN = 3    # T路口邊界判斷閥值下限(px，依據線寬調整)
...
def flight_speed_controller():
    ...
    while STATE_THREAD:
        ...
        while MODE == 'LINE' and STATE_THREAD and not SPD_MODE:
            ...
            t_on = time.time()
            # 輸出至全域變數(無覆寫旗標時)
            if SPD_OL_ACTIVE_TIME != 0 and not SPD_PITCH_OVER_WRITE:
            ...
            t_off = time.time()
            # 輸出至全域變數(無覆寫旗標時)
            if SPD_OL_OFF_TIME != 0 and not SPD_PITCH_OVER_WRITE:
                TARGET_PITCH = 0
...
def camera():
    global CAM_IMAGE, CAM_IMAGE_2, STATE_CAM, STATE_THREAD
    # 程式迴圈(同捆執行模式)
...
# ========== 色塊面積回傳 ===============================================
# 專門用於程序判斷呼叫用條件，與背景者分離，使用線程鎖定
def color_area(color):
    """計算指定顏色型心位置、色塊大小，支援紅、綠、藍、黑色"""
    THREAD_OCCUPY.acquire()
    # 色域轉換BGR->HSV
    image_hsv = cv2.cvtColor(CAM_IMAGE_2, cv2.COLOR_BGR2HSV)
    # 閉運算
    image_close = cv2.morphologyEx(image_hsv, cv2.MORPH_CLOSE,
                                   np.ones(IMG_MORPHOLOGY_EX_KERNEL, np.uint8))
    ...
    # 計算面積一次矩
    for i in range(len(area_register)):
        area_sum += area_register[i]
...
def thread_activate():
    global STATE_THREAD, STATE_UAV, uav, CAM_VIDEO
    # 若未連線，重新執行連線
    if not STATE_UAV:
        print('<啟動程序> 與UAV連線中，請稍候...')
        # 樹莓派連線指令
...
# ========== 變高程序 =================================================
def change_alt(target_alt):
    """調整高度指令：更改飛行高度(程序佔用)"""
    global TARGET_ALT
    TARGET_ALT = target_alt
    print('<飛行命令> 變更高度至 %.2f m' % TARGET_ALT)

    # 等待抵達目標高度
    i = 0
    if TARGET_ALT > ACTUAL_ALT:
        while ACTUAL_ALT < TARGET_ALT * (TAKEOFF_SETTLE / 100):
            # 顯示飛行資訊
            if i % 5 == 0:
                print('<飛行狀態> 高度調整中，目前高度 %.2f m，距離目標高度尚有 %.2f m' %
                      (ACTUAL_ALT, TARGET_ALT - ACTUAL_ALT))
                flight_info()
    ...
    elif TARGET_ALT < ACTUAL_ALT:
        while ACTUAL_ALT > TARGET_ALT * (100 / TAKEOFF_SETTLE):
            # 顯示飛行資訊
            if i % 5 == 0:
                print('<飛行狀態> 高度調整中，目前高度 %.2f m，距離目標高度尚有 %.2f m' %
                      (ACTUAL_ALT, TARGET_ALT - ACTUAL_ALT))
...
def mode_line(spd_type=0, flight_speed=0.0):
    ...
    # 設定飛行作用俯仰角
        ...
        print('<飛行狀態> 切換至開路循跡模式，目標作動角度 %d degree，開始往前飛行\n' % TARGET_SPD)
    THREAD_OCCUPY.release()
...
def wait_leave(color):
    ...
    global TARGET_PITCH, SPD_PITCH_OVER_WRITE
    # 初始化顏色面積
    img_area = 100000
    ...
    while img_area > AREA_LEAVE_LIMIT and MODE == 'LINE':
        ...
        # 初始推動角(1.5s)
        if i < 15:
            SPD_PITCH_OVER_WRITE = True
            TARGET_PITCH = -4
        else:
            SPD_PITCH_OVER_WRITE = False
...
def wait_t_intersection():
    ...
    global ANG_T_TURN, TARGET_PITCH, SPD_PITCH_OVER_WRITE
    # 一般循線段
    i = 0
    ...
    TARGET_PITCH = -3
    red_area = 0
    while red_area < T_RED_ARRIVE_LIMIT:
        # 更新面積數值
        red_area = color_area('red')
...
def turn():
    ...
    while 1:        # 藍右，綠左
        # 讀取顏色面積值做判斷
        image_area_blue = color_area('blue')
        image_area_green = color_area('green')
...
def drop(color=MISSION_COLOR):
    ...
    while 1:
        ...
        # 判斷是否已抵達中心
        if -DROP_POS_LIMIT_X < ACTUAL_POS[0] < DROP_POS_LIMIT_X:
            if -DROP_POS_LIMIT_Y < ACTUAL_POS[1] < DROP_POS_LIMIT_Y:
                break
...
def mission():
    ...
    if mission_mode == 4:
        ...
        mode_line(0, 3)         # 循跡模式
        wait_leave(MISSION_COLOR)       # 離開色塊指令(強制推出)
        wait_arrive(MISSION_COLOR)      # 等待抵達
        drop(MISSION_COLOR)             # 定位投擲

        mode_line(0, 3)                 # 循跡模式
        wait_leave(MISSION_COLOR)       # 離開色塊指令(強制推出)
        wait_t_intersection()           # T偵測與減速轉彎推入降落區

        wait_arrive(MISSION_COLOR)      # 偵測抵達降落區
        landing(MISSION_COLOR)          # 執行降落與關閉程序
