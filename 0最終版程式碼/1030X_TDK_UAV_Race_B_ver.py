...
# 定位定高皆已通過驗證測試(穩定性90％)
# L轉彎後色塊抓取通過(紅色)、沙包盒功能測試通過
# 需執行循線校正：Roll軸追蹤暫態與穩態調整、T字路口邏輯處理
# 各區測試、總成測試、燈號旋轉測試需進行
...
ARDUINO_CONNECT_BAUD = 115200               # 傳輸鮑率
...
# ->循跡-滾轉控制器參數(比照定位X軸處理)
# 外迴圈
HDG_X_KP = 0.6                  # 位置控制器P增益值(回至中心線速度)*
HDG_X_KI = 0                    # 位置控制器I增益值(有穩差才使用)*
HDG_X_KD = 0                    # 位置控制器D增益值(減少震盪並拉快安定時間)*
HDG_X_DZ = 5                    # 位置控制器輸入截止區參數(於截止區內使速度控制器控制零速度，防震盪)
HDG_SAMPLE_NUM_AVE = 10         # 位置控制器均值濾波取樣次數*
# 內迴圈
HDG_VX_KP = 0.04                # 速度控制器P增益值(回至中心線速度)*
HDG_VX_KI = 0                   # 速度控制器I增益值(有穩差才使用)*
HDG_VX_KD = 0.03                # 速度控制器D增益值(減少震盪並拉快安定時間)*
HDG_ROLL_LIMIT = 4              # 速度控制器-最大飛行角度(degree)
HDG_ROLL_CUTOFF = 0.5           # 速度控制器-最小飛行角度(degree)
HDG_VX_KH = 0.35                # X軸回授增益(與控制器增益有相依性)
...
ANG_PERPENDICULAR_HEIGHT = 15   # 橫線角度判定高度
...
LASER_SAMPLE_NUM_CAL = 5        # 高度校正取樣次數*
...
def altitude_sensor():
    ...
    while STATE_THREAD:
        ...
        R_ACTUAL_ALT_LASER.append(height_laser)         # 記錄雷射測距儀值
...
# ========== 感測器接收程式 ==============================================
# 說明：
# 負責從Arduino接收雷射高度值、三軸加速規數值，並做補正與濾波後寫至全域變數
def get_sensor_data():
    global ACTUAL_ALT_LASER, ACTUAL_AX, ACTUAL_AY, ACTUAL_AZ, STATE_SENSOR
    # 變數初始化
    alt_laser_raw = 0
    ax = 0
    ay = 0
    ...
    while STATE_THREAD:
        ...
        # 雷射測距儀高度與角度補正
        alt_laser_offset = ((float(alt_laser_raw)/1000) * math.cos(uav.attitude.roll) *
                            math.cos(uav.attitude.pitch)) + LASER_OFFSET

        # 雷射測距儀均值濾波器
        height_register_laser.insert(0, alt_laser_offset)
        ...
        # 迴圈執行間隔
        t_wait = (1 / FREQ_SENSOR) - (time.time() - t_loop)
        if t_wait > 0:
            time.sleep(t_wait)

    # 當多線程狀態為否時，跳出迴圈並關閉函式
    STATE_SENSOR = False
    print('<關閉程序> 感測器接收程式：關閉')
...
def laser_calibrate():
    ...
    for i in range(0, LASER_SAMPLE_NUM_CAL):
        ...
        # 資料型態轉換
        height_raw = float(alt_laser)/1000
...
def line_moment():
    ...
    for c in contours:
        ...
        if block_area >= ANG_LINE_TRACK_AREA_L:
            ...
            if h == 0:
                line_angle = 0
            elif h <= ANG_PERPENDICULAR_HEIGHT:
                if ACTUAL_HDG == 0:
                    line_angle = 0
                elif ACTUAL_HDG > 0:
                    line_angle = 90
                ...
                if img_th[y, x] == 255 and img_th[y+w-1, x+h-1] == 255:   # T路口邏輯
                    if ACTUAL_HDG > 0:
                        line_angle *= 1
                    elif ACTUAL_HDG < 0:
                        line_angle *= -1
...
def thread_activate():
    ...
    get_sen = threading.Thread(target=get_sensor_data)
...
def flight_live_info():
    ...
    print('<高度狀態> 總高：%.2f m，超音波：%.2f m，IMU：%.2f m，雷射：%.2f m，輸出THR：%.1f%%' %
          (ACTUAL_ALT, ACTUAL_ALT_SONAR, ACTUAL_ALT_IMU, ACTUAL_ALT_LASER, TARGET_THRUST*100))
