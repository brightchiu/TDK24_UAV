...
# ========== Arduino連線參數 ==========================================
ARDUINO_CONNECT_PORT = '/dev/ttyUSB0'       # 傳輸口位址
...
POS_X_KD = 0.03                 # 位置控制器X軸-D增益值
POS_Y_KP = 0.15                  # 位置控制器Y軸-P增益值
POS_Y_KI = 0                    # 位置控制器Y軸-I增益值
POS_Y_KD = 0.6                  # 位置控制器Y軸-D增益值
POS_X_DZ = 100                  # 位置控制器輸入截止區參數(於截止區內使速度控制器控制零速度，防震盪)
POS_Y_DZ = 75                  # 位置控制器輸入截止區參數(於截止區內使速度控制器控制零速度，防震盪)
POS_SAMPLE_NUM_AVE = 10         # 位置控制器均值濾波取樣次數*
# 特殊附註：截止區調校方式=>區域內阻力可以抵消慣性、控制器解析程度內(控制器截止區)

# ->位置-速度控制器參數
VEL_X_KP = 0.13                  # 速度控制器X軸-P增益值
...
VEL_X_KH = 0.06                  # X軸回授增益
VEL_Y_KH = 0.12                  # Y軸回授增益
...
HDG_YAW_LINE_DIS_LIMIT = 25     # 角度限制作動距離解除區(px)

# ->循跡-滾轉控制器參數
HDG_ROLL_KP = 0.1              # P增益值(回至中心線速度)*
HDG_ROLL_KI = 0                 # I增益值(有穩差才使用)*
HDG_ROLL_KD = 0.1              # D增益值(減少震盪並拉快安定時間)*
HDG_ROLL_INT_LIMIT = 1          # 積分上限(配合I控制調整)
HDG_ROLL_DZ = 25                # 截止區距離(px，依照滑行減速能力調整)*
HDG_ROLL_ANGLE_LIMIT = 2        # 角度輸出限制(degree)*
HDG_ROLL_ANGLE_CUTOFF = 0.5     # 角度截止限制(degree)*

# ->飛行速度控制器參數-閉迴路
SPD_MODE = 0                    # 飛行模式(1:閉迴路模式 0:開迴路模式)
SPD_KP = 1                      # P增益值*
SPD_KI = 0                      # I增益值*
SPD_KD = 0                      # D增益值*
SPD_ANGLE_CUTOFF = 0.5          # 飛行速度控制器-最小俯仰角度(degree)
SPD_ANGLE_LIMIT = 1.5             # 飛行速度控制器-最大俯仰角度(degree)
SPD_OFF_ANGLE = 5               # 切換跳出角度(degree)，慣性抵銷用
SPD_SAMPLE_NUM_AVE = 5          # 飛行速度感測器均值濾波取樣次數*

# ->飛行速度控制器參數-開迴路
SPD_OL_ACTIVE_TIME = 1          # 開迴路控制模式，角度作動時長(s)
SPD_OL_OFF_TIME = 10             # 開迴路控制模式，角度關閉時長(s)
...
# ========== 高度感測器參數 =============================================
# ->高度感測器整合參數
WEIGHTED_SONAR = 3              # 超音波高度計權重*
WEIGHTED_IMU = 1                # IMU高度計權重*
WEIGHTED_LASER = 0              # 雷射測距儀權重*
...
# 連線指令
# t_start = time.time()
# nano = serial.Serial(port=ARDUINO_CONNECT_PORT, baudrate=ARDUINO_CONNECT_BAUD, timeout=0.05)
# nano.open()
# t_end = time.time()
...
def position_controller():
    ...
    while STATE_THREAD:
        while MODE == 'POS' and STATE_THREAD:
            ...
            if TARGET_POS[1] - POS_X_DZ < pos_actual_y < TARGET_POS[1] + POS_X_DZ:
...
def heading_controller():
    ...
    while STATE_THREAD:
        ...
        while MODE == 'LINE' and STATE_THREAD:
            ...
            # 讀取回授值
            line_pos_actual = ACTUAL_LINE_POS
            hdg_actual = ACTUAL_HDG

            # 回授值截止區修正
            if HDG_ROLL_DZ > line_pos_actual > -HDG_ROLL_DZ:
                line_pos_actual = 0
            if HDG_YAW_DZ > hdg_actual > -HDG_YAW_DZ:
                hdg_actual = 0

            # 計算誤差值
            error_roll = 0 - line_pos_actual
            error_yaw = TARGET_HDG - hdg_actual

            # 誤差值微積分計算
            current_time = time.time()
            delta_time = current_time - previous_time

            delta_error_roll = error_roll - previous_error_roll
            int_error_roll += (error_roll * delta_time)
            derivative_roll = delta_error_roll / delta_time

            delta_error_yaw = error_yaw - previous_error_yaw
            int_error_yaw += (error_yaw * delta_time)
            derivative_yaw = delta_error_yaw / delta_time

            # X軸誤差增益處理
            gain_p_roll = error_roll * HDG_ROLL_KP
            gain_i_roll = int_error_roll * HDG_ROLL_KI
            gain_d_roll = derivative_roll * HDG_ROLL_KD
            roll_out = gain_p_roll + gain_i_roll + gain_d_roll

            # Y軸誤差增益處理
            gain_p_yaw = error_yaw * HDG_YAW_KP
            gain_i_yaw = int_error_yaw * HDG_YAW_KI
            gain_d_yaw = derivative_yaw * HDG_YAW_KD
            yaw_rate_out = gain_p_yaw + gain_i_yaw + gain_d_yaw
            ...
            # 輸出限制(當線達角度限制，且型心抵達中心區才作動
            if ((math.fabs(ACTUAL_LINE_ANG) > HDG_YAW_LINE_ANGLE_LIMIT) and not
                    (HDG_YAW_LINE_DIS_LIMIT > ACTUAL_LINE_POS > -HDG_YAW_LINE_DIS_LIMIT)):
                yaw_rate_out = 0
            ...
            R_HDG_ROLL_ERROR.append(error_roll)
            R_HDG_ROLL_GP.append(gain_p_roll)
            R_HDG_ROLL_GI.append(gain_i_roll)
            R_HDG_ROLL_GD.append(gain_d_roll)
            ...
            R_HDG_YAW_ERROR.append(error_yaw)
            R_HDG_YAW_GP.append(gain_p_yaw)
            R_HDG_YAW_GI.append(gain_i_yaw)
            R_HDG_YAW_GD.append(gain_d_yaw)

            # 同捆執行終點
            THREAD_OCCUPY.release()

            # 迴圈執行間隔
            t_wait = (1 / FREQ_HDG) - (time.time() - t_loop)
            if t_wait > 0:
                time.sleep(t_wait)
...
def altitude_sensor():
    ...
    while STATE_THREAD:
        ...
        R_ACTUAL_ALT_LASER.append(0)                    # 記錄雷射測距高度
        R_ACTUAL_ALT_LASER_RAW.append(0)                # 記錄雷射測距高度原始值
        R_ACTUAL_ALT_LASER_OFFSET.append(0)             # 記錄雷射測距高度補正值
...
def get_sensor_data():
    ...
    while STATE_THREAD:
        ...
        # 切出資料
        sen_data = sen_raw_data.split(sep='.')
        # 抓取資料
...
def take_off(target_alt=1.0, color=''):
    ...
    while ACTUAL_ALT < TARGET_ALT * (TAKEOFF_SETTLE / 100):
        ...
        if i % 5 == 0:
            ...
            print('<輸出命令狀態> T-roll:%.2f  T-pitch:%.2f' % (TARGET_ROLL, TARGET_PITCH))
...
def turn():
    ...
    while 1:  # 藍右，綠左
        ...
        # 若偵測到藍色
        if i > 100:   #image_area_blue >= TURN_AREA_LIMIT:
            # 記錄任務顏色並更改定位顏色
            MISSION_COLOR = 'red'
