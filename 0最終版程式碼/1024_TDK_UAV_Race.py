...
# 檔案日期 1024 航向、飛速、資料區、任務區、沙盒區未確認 1025
# 測試情形：
...
ACTUAL_LINE_ANG = 0             # 線角度(degree，Y軸為基準，最大轉角限制用)
ACTUAL_LINE_POS = 0             # 線距離(px，Y軸為基準，Roll控制器回授)
...
FREQ_POS = 15                   # 位置控制器頻率(不快於位置-速度控制器頻率)
FREQ_VEL = 30                   # 位置-速度控制器頻率(不快於影像禎率)
...
# ->位置控制器參數
POS_X_KP = 0.06                 # 位置控制器X軸-P增益值
POS_X_KI = 0                    # 位置控制器X軸-I增益值
POS_X_KD = 0.06                 # 位置控制器X軸-D增益值
POS_Y_KP = 0.1                  # 位置控制器Y軸-P增益值
POS_Y_KI = 0                    # 位置控制器Y軸-I增益值
POS_Y_KD = 0.1                  # 位置控制器Y軸-D增益值
POS_X_DZ = 100                  # 位置控制器輸入截止區參數(於截止區內使速度控制器控制零速度，防震盪)
POS_Y_DZ = 100                  # 位置控制器輸入截止區參數(於截止區內使速度控制器控制零速度，防震盪)
...
VEL_X_KD = 0                    # 速度控制器X軸-D增益值
VEL_Y_KP = 0.25                 # 速度控制器Y軸-P增益值
VEL_Y_KI = 0                    # 速度控制器Y軸-I增益值
VEL_Y_KD = 0                    # 速度控制器Y軸-D增益值
VEL_ANGLE_LIMIT = 3             # 速度控制器-最大飛行角度(degree)
VEL_ANGLE_CUTOFF = 0.5          # 速度控制器-最小飛行角度(degree)
VEL_X_KH = 0.1                  # X軸回授增益
VEL_Y_KH = 0.1                  # Y軸回授增益
VEL_CUTOFF_ALT = 0.5            # 控制器最低作動高度(m，飽和高度點)

# ->循跡-航向控制器參數
HDG_YAW_KP = 0.1                # P增益值*
HDG_YAW_KI = 0                  # I增益值*
HDG_YAW_KD = 0                  # D增益值*
HDG_YAW_DZ = 5                  # 截止區角度(degree)*
HDG_YAW_RATE_LIMIT = 20         # 角速度輸出限制(degree/s)*
HDG_YAW_LINE_ANGLE_LIMIT = 30   # 與線最大夾角限制(degree)*

# ->循跡-滾轉控制器參數
HDG_ROLL_KP = 0.06              # P增益值(回至中心線速度)*
HDG_ROLL_KI = 0                 # I增益值(有穩差才使用)*
HDG_ROLL_KD = 0.06              # D增益值(減少震盪並拉快安定時間)*
HDG_ROLL_INT_LIMIT = 1          # 積分上限(配合I控制調整)
HDG_ROLL_DZ = 25                # 截止區距離(px，依照滑行減速能力調整)*
HDG_ROLL_ANGLE_LIMIT = 3        # 角度輸出限制(degree)*
HDG_ROLL_ANGLE_CUTOFF = 0.5     # 角度截止限制(degree)*
...
# ->飛行速度控制器參數-開迴路
SPD_OL_ACTIVE_TIME = 2          # 開迴路控制模式，角度作動時長(s)
SPD_OL_OFF_TIME = 8             # 開迴路控制模式，角度關閉時長(s)
...
BODY_OFFSET_Y = 0                           # 機身中心Y偏移量(px，中心往下移正)*
...
SONAR_KF_CHANGE_RATE_MAX = 3    # 卡爾曼濾波器輸入變動率限制(%)*
...
# ->航向控制器
R_HDG_ROLL_ACTUAL_X = []                # 線X座標
R_HDG_ROLL_TARGET_ROLL = []             # 記錄目標滾轉
R_HDG_ROLL_ACTUAL_ROLL = []             # 記錄實際滾轉角度
R_HDG_ROLL_ERROR = []                   # 記錄距離誤差
R_HDG_ROLL_GP = []                      # 記錄P增益
R_HDG_ROLL_GI = []                      # 記錄I增益
R_HDG_ROLL_GD = []                      # 記錄D增益
R_HDG_YAW_ACTUAL = []                   # 記錄實際角度
R_HDG_YAW_ACTUAL_LINE = []              # 線角度
...
R_ACTUAL_ALT_LASER_RAW = []             # 記錄雷射測距原始值
R_ACTUAL_ALT_LASER_OFFSET = []          # 記錄雷射測距補正值
...
# ========== 控制器物件 ================================================
class PidController:
    """PID控制器，可控制PID增益、回授增益、輸入截止、輸出區間、定時迴圈"""
    # 載入時間模組
    time = __import__('time')

    # 初始化設定
    def __init__(self, kp=0.0, ki=0.0, kd=0.0, kh=1.0, frequency=20):
        self.frequency = frequency          # 控制器更新頻率
        self.reference = 0                  # 參考值
        self.feedback = 0                   # 回授值
        self.error = 0                      # 誤差值
        self.error_integral = 0             # 累積誤差
        self.error_previous = 0             # 前次誤差
        self.kp = kp                        # 比例增益
        self.ki = ki                        # 積分增益
        self.kd = kd                        # 微分增益
        self.kh = kh                        # 回授增益
        self.gain_p = 0                     # 比例增益輸出
        self.gain_i = 0                     # 積分增益輸出
        self.gain_d = 0                     # 微分增益輸出
        self.gain_h = 0                     # 回授增益輸入
        self.limit_integral = 1000000       # 積分限制
        self.limit_gain_d = 1000000         # 微分輸出限制
        self.limit_output = 1000000         # 輸出限制
        self.limit_output_cutoff = 0        # 輸出截止限制
        self.limit_input_cutoff = 0         # 輸入截止限制
        self.time_previous = self.time.time()   # 上次時刻
        self.time_current = self.time.time()    # 本次時刻
        self.time_start = self.time.time()      # 迴圈起始時刻
        self._first_loop = True             # 初始迴圈旗標
        self._sleep = False                 # 睡眠旗標

    # 設定增益限制
    def set_limit_id(self, limit_integral=1000000, limit_gain_d=1000000):
        """設定微積分限制"""
        self.limit_integral = limit_integral
        self.limit_gain_d = limit_gain_d

    # 設定輸入限制
    def set_limit_input(self, limit_input_cutoff=0.0):
        """設定輸入限制"""
        self.limit_input_cutoff = limit_input_cutoff

    # 設定輸出限制
    def set_limit_output(self, limit_output=1000000.0, limit_output_cutoff=0.0):
        """設定輸出限制"""
        self.limit_output = limit_output
        self.limit_output_cutoff = limit_output_cutoff
    ...
    # 更新目標值
    def update_controller(self, reference, feedback):
        """更新控制器目標與回授值"""
        # 更新輸入與回授
        ...
        self.time_current = self.time.time()

    # 控制器輸出
    def control_process(self):
        """更新控制器輸出"""
        # 第一迴圈圈僅作時間更新
        if self._first_loop or self._sleep:
            ...
            # 輸入截止限制
            if -self.limit_input_cutoff < self.feedback < self.limit_input_cutoff:
                self.error = 0
            # 一般狀態
            else:
                # 回授增益
                self.gain_h = self.feedback * self.kh
                # 計算誤差值
                self.error = self.reference - self.gain_h

            # 誤差值微積分計算
            delta_time = self.time_current - self.time_previous
            delta_error = self.error - self.error_previous
            self.error_integral += (self.error * delta_time)
            derivative = delta_error / delta_time

            # 積分器限制
            if self.error_integral > self.limit_integral:
                self.error_integral = self.limit_integral
            elif self.error_integral < -self.limit_integral:
                self.error_integral = -self.limit_integral

            # 誤差增益處理
            self.gain_p = self.error * self.kp
            self.gain_i = self.error_integral * self.ki
            self.gain_d = derivative * self.kp

            # 微分增益限制器
            if self.gain_d > self.limit_gain_d:
                self.gain_d = self.limit_gain_d
            elif self.gain_d < -self.limit_gain_d:
                self.gain_d = -self.limit_gain_d
            ...
            # 輸出、截止限制器
            if gain_out > self.limit_output:
                gain_out = self.limit_output
            elif gain_out < -self.limit_output:
                gain_out = -self.limit_output
            elif self.limit_output_cutoff > gain_out > -self.limit_output_cutoff:
                gain_out = 0
    ...
    def wait_next_loop(self):
        ...
        time_current = self.time.time()
        # 計時迴圈
        while time_current - self.time_start < time_elapsed:
            time_current = self.time.time()
        # 更新計時器
...
# ========== 卡爾曼濾波器物件 ===========================================
class KalmanFilter:
    """卡爾曼濾波器，初始化設定降噪能力與容許變化率"""
    # 載入數學模組
    math = __import__('math')

    def __init__(self, kf_q=5, input_rate=0.5):
        # 卡爾曼濾波器初始化
        self.x_o = 0                    # 前一次狀態
        self.p_o = 0                    # 前一次斜方差
        self.z_o = 0                    # 前一次量測值
        self.q = self.math.exp(-kf_q)               # 斜方差噪音值
        self.r = 2.92 * self.math.exp(-3)           # 斜方差量測噪音值(定值不更改)
        self.limit_input_change = input_rate        # 最大容許輸入變化率

    def processing(self, measure):
        """濾波執行函式"""
        # 卡爾曼濾波-預測
        x_p = self.x_o
        p_p = self.p_o + self.q
        ...
        # 輸入限制器
        delta_measure = measure - self.z_o
        if self.z_o > 0.5 and self.math.fabs(delta_measure/self.z_o) > self.limit_input_change:
            z = self.z_o                # 當輸入變化超出幅度時，使用上筆資料
        else:
            z = measure

        # 卡爾曼濾波-更新
        k = p_p / (p_p + self.r)        # 卡爾曼增益
...
def altitude_controller():
    ...
    # 建立控制器物件
    alt_ctrl = PidController(kp=ALT_KP, ki=ALT_KI, kd=ALT_KD, frequency=FREQ_ALT)
    ...
    while STATE_THREAD:
        ...
        # 更新控制器
        alt_ctrl.update_controller(TARGET_ALT, ACTUAL_ALT)

        # 將速度以比例輸出
        thrust_out = alt_ctrl.control_process() / ALT_SPEED
...
def position_controller():
    ...
    # 建立濾波器
    vel_x_register = MovingAverageFilter(10)
    vel_y_register = MovingAverageFilter(10)

    # 建立控制器物件
    pos_x_ctrl = PidController(kp=POS_X_KP, ki=POS_X_KI, kd=POS_X_KD, frequency=FREQ_POS)
    pos_y_ctrl = PidController(kp=POS_Y_KP, ki=POS_Y_KI, kd=POS_Y_KD, frequency=FREQ_POS)
    pos_x_ctrl.set_limit_input(limit_input_cutoff=POS_X_DZ)
    pos_y_ctrl.set_limit_input(limit_input_cutoff=POS_Y_DZ)
    ...
    while STATE_THREAD:
        while MODE == 'POS' and STATE_THREAD:
            ...
            # 更新控制器
            pos_x_ctrl.update_controller(TARGET_POS[0], ACTUAL_POS[0])
            pos_y_ctrl.update_controller(TARGET_POS[1], ACTUAL_POS[1])

            # 執行控制器輸出
            vel_x_out_raw = pos_x_ctrl.control_process()
            vel_y_out_raw = pos_y_ctrl.control_process()

            # 均值濾波器
            vel_x_out = vel_x_register.calculate(vel_x_out_raw)
            vel_y_out = vel_y_register.calculate(vel_y_out_raw)
            ...
            # 迴圈執行間隔(僅需呼叫一個)
            pos_x_ctrl.wait_next_loop()

            # 當飛行模式不為定位時，跳出迴圈並暫停控制器
            if not MODE == 'POS':
                # 控制器休眠
                pos_x_ctrl.set_sleep()
                pos_y_ctrl.set_sleep()

                # 重設濾波器
                vel_x_register.reset_register()
                vel_y_register.reset_register()
...
def position_velocity_controller():
    ...
    # 建立控制器物件
    vel_x_ctrl = PidController(kp=VEL_X_KP, ki=VEL_X_KI, kd=VEL_X_KD, kh=VEL_X_KH, frequency=FREQ_VEL)
    vel_y_ctrl = PidController(kp=VEL_Y_KP, ki=VEL_Y_KI, kd=VEL_Y_KD, kh=VEL_Y_KH, frequency=FREQ_VEL)
    vel_x_ctrl.set_limit_output(limit_output=VEL_ANGLE_LIMIT, limit_output_cutoff=VEL_ANGLE_CUTOFF)
    vel_y_ctrl.set_limit_output(limit_output=VEL_ANGLE_LIMIT, limit_output_cutoff=VEL_ANGLE_CUTOFF)
    ...
    while STATE_THREAD:
        while MODE == 'POS' and STATE_THREAD:
            ...
            # 更新控制器
            vel_x_ctrl.update_controller(TARGET_VEL[0], ACTUAL_VEL[0])
            vel_y_ctrl.update_controller(TARGET_VEL[1], ACTUAL_VEL[1])

            # 執行控制器輸出
            roll_out = vel_x_ctrl.control_process()
            pitch_out = vel_y_ctrl.control_process()

            # 輸出至全域變數
            if ACTUAL_ALT < VEL_CUTOFF_ALT:
                TARGET_ROLL = 0
                TARGET_PITCH = 0
            else:
            ...
            R_POS_VEL_X_ACT.append(vel_x_ctrl.gain_h)
            R_POS_VEL_Y_ACT.append(vel_y_ctrl.gain_h)
            ...
            # 迴圈執行間隔
            vel_x_ctrl.wait_next_loop()

            # 當飛行模式不為定位時，跳出迴圈並暫停函式
            if not MODE == 'POS':
                # 控制器休眠
                vel_x_ctrl.set_sleep()
                vel_y_ctrl.set_sleep()
...
def heading_controller():
    ...
    # 建立控制器物件
    roll_ctrl = PidController(kp=HDG_ROLL_KP, ki=HDG_ROLL_KI, kd=HDG_ROLL_KD, frequency=FREQ_HDG)
    yaw_ctrl = PidController(kp=HDG_YAW_KP, ki=HDG_YAW_KI, kd=HDG_YAW_KD, frequency=FREQ_HDG)

    roll_ctrl.set_limit_input(limit_input_cutoff=HDG_ROLL_DZ)
    roll_ctrl.set_limit_output(limit_output=HDG_ROLL_ANGLE_LIMIT, limit_output_cutoff=HDG_ROLL_ANGLE_CUTOFF)
    roll_ctrl.set_limit_id(limit_integral=HDG_ROLL_INT_LIMIT)

    yaw_ctrl.set_limit_input(limit_input_cutoff=HDG_YAW_DZ)
    yaw_ctrl.set_limit_output(limit_output=HDG_YAW_RATE_LIMIT)
    ...
    while STATE_THREAD:
        ...
        while MODE == 'LINE' and STATE_THREAD:
            ...
            # 更新控制器
            roll_ctrl.update_controller(0, ACTUAL_LINE_POS)
            yaw_ctrl.update_controller(TARGET_HDG, ACTUAL_HDG)

            # 執行控制器輸出
            roll_out = roll_ctrl.control_process()
            yaw_rate_out = yaw_ctrl.control_process()

            # 輸出限制
            if math.fabs(ACTUAL_LINE_ANG) > HDG_YAW_LINE_ANGLE_LIMIT:
                yaw_rate_out = 0
            ...
            # 記錄數值
            R_HDG_ROLL_ACTUAL_X.append(ACTUAL_LINE_POS)
            R_HDG_ROLL_TARGET_ROLL.append(TARGET_ROLL)
            R_HDG_ROLL_ACTUAL_ROLL.append(math.degrees(uav.attitude.roll))
            R_HDG_ROLL_ERROR.append(roll_ctrl.error)
            R_HDG_ROLL_GP.append(roll_ctrl.gain_p)
            R_HDG_ROLL_GI.append(roll_ctrl.gain_i)
            R_HDG_ROLL_GD.append(roll_ctrl.gain_d)

            R_HDG_YAW_ACTUAL.append(ACTUAL_HDG)
            R_HDG_YAW_ACTUAL_LINE.append(ACTUAL_LINE_ANG)
            R_HDG_YAW_TARGET_YAW_RATE.append(TARGET_YAW_RATE)
            R_HDG_YAW_ERROR.append(yaw_ctrl.error)
            R_HDG_YAW_GP.append(yaw_ctrl.gain_p)
            R_HDG_YAW_GI.append(yaw_ctrl.gain_i)
            R_HDG_YAW_GD.append(yaw_ctrl.gain_d)

            # 同捆執行終點
            THREAD_OCCUPY.release()

            # 迴圈執行間隔
            roll_ctrl.wait_next_loop()
        ...
        # 記錄數值
        R_HDG_ROLL_ACTUAL_X.append(0)
        R_HDG_ROLL_TARGET_ROLL.append(0)
        R_HDG_ROLL_ACTUAL_ROLL.append(0)
        R_HDG_ROLL_ERROR.append(0)
        R_HDG_ROLL_GP.append(0)
        R_HDG_ROLL_GI.append(0)
        R_HDG_ROLL_GD.append(0)

        R_HDG_YAW_ACTUAL.append(0)
        R_HDG_YAW_ACTUAL_LINE.append(0)
...
def flight_speed_controller():
    ...
    # 建立控制器物件
    spd_ctrl = PidController(kp=SPD_KP, ki=SPD_KI, kd=SPD_KD, frequency=FREQ_SPD)
    spd_ctrl.set_limit_output(limit_output=SPD_ANGLE_LIMIT, limit_output_cutoff=SPD_ANGLE_CUTOFF)
    ...
    while STATE_THREAD:
        ...
        while MODE == 'LINE' and STATE_THREAD and SPD_MODE:
            ...
            # 更新控制器
            spd_ctrl.update_controller(TARGET_SPD, ACTUAL_SPD)

            # 執行控制器輸出
            pitch_out = spd_ctrl.control_process()
...
def camera_sensor():
    ...
    global ACTUAL_HDG, ACTUAL_LINE_ANG, ACTUAL_LINE_POS
    # 均值濾波器設定
    pos_x_register = MovingAverageFilter(IMG_SAMPLE_NUM_AVE)
    pos_y_register = MovingAverageFilter(IMG_SAMPLE_NUM_AVE)
    vel_x_register = MovingAverageFilter(IMG_SAMPLE_NUM_AVE)
    vel_y_register = MovingAverageFilter(IMG_SAMPLE_NUM_AVE)
    hdg_register = MovingAverageFilter(ANG_SAMPLE_NUM_AVE)
    line_angle_register = MovingAverageFilter(ANG_SAMPLE_NUM_AVE)
    ...
    while STATE_THREAD:
        ...
        while MODE == 'POS' and STATE_THREAD:
            # 確認已經完成重設
            if _first_loop:
                pos_x_register.reset_register()
                pos_y_register.reset_register()
                vel_x_register.reset_register()
                vel_y_register.reset_register()
                hdg_register.reset_register()
                line_angle_register.reset_register()
                _first_loop = False
            ...
            # 位置均值濾波器
            pos_x = pos_x_register.calculate(x_raw)
            pos_y = pos_y_register.calculate(y_raw)
            ...
            # 速度均值濾波器
            vel_x = vel_x_register.calculate(vel_x_raw)
            vel_y = vel_y_register.calculate(vel_y_raw)
            ...
            # 迴圈執行間隔
            while time.time() - t_loop < (1 / FREQ_SENSOR_CAM):
                pass
            t_loop = time.time()
        ...
        while MODE == 'LINE' and STATE_THREAD:
            ...
            # 型心位置均值濾波器
            pos_x = pos_x_register.calculate(pos_x_raw)
            pos_y = pos_y_register.calculate(pos_y_raw)

            # 取得現在角度值(中心為目標值，朝右正，朝左負，使用global Frame參照)
            hdg_raw = angle_calculate(pos_x, pos_y)

            # 誤差角均值濾波器
            hdg = hdg_register.calculate(hdg_raw)
            line_angle = line_angle_register.calculate(line_angle_raw)

            # 輸出角度誤差
            ACTUAL_HDG = hdg
            ACTUAL_LINE_ANG = line_angle
...
def altitude_sensor():
    ...
    # 濾波器設定
    height_register_imu = MovingAverageFilter(IMU_SAMPLE_NUM_AVE)
    height_register_imu_kf = KalmanFilter(kf_q=IMU_KF_Q, input_rate=IMU_KF_CHANGE_RATE_MAX)
    height_register_laser = MovingAverageFilter(LASER_SAMPLE_NUM_AVE)
    ...
    while STATE_THREAD:
        ...
        height_raw_laser = ((laser.get_distance() / 1000) * math.cos(roll_angle) * math.cos(pitch_angle))
        height_offset_laser = height_raw_laser + LASER_OFFSET

        # IMU濾波器
        height_imu_ave = height_register_imu.calculate(height_raw_imu)
        height_imu = height_register_imu_kf.processing(height_imu_ave)

        # 雷射測距儀均值濾波器
        height_laser = height_register_laser.calculate(height_offset_laser)

        # 更新高度值至全域變數
        ACTUAL_ALT_IMU = height_imu
        ACTUAL_ALT_LASER = height_laser

        # 加權平均(可調整加權值)
        alt_num = (WEIGHTED_SONAR * height_sonar + WEIGHTED_IMU * height_imu
                   + WEIGHTED_LASER * height_laser)
        ...
        R_ACTUAL_ALT_LASER.append(height_laser)         # 記錄雷射測距高度
        R_ACTUAL_ALT_LASER_RAW.append(height_raw_laser)                         # 記錄雷射測距高度原始值
        R_ACTUAL_ALT_LASER_OFFSET.append(height_offset_laser)                   # 記錄雷射測距高度補正值
        ...
        # 等待下次迴圈
        while time.time()-t_loop < (1 / FREQ_SENSOR_ALT):
            pass
        t_loop = time.time()
...
def altitude_sensor_sonar():
    ...
    # 濾波器設定
    height_register = MovingAverageFilter(SONAR_SAMPLE_NUM_AVE)
    height_register_kf = KalmanFilter(kf_q=SONAR_KF_Q, input_rate=SONAR_KF_CHANGE_RATE_MAX)
    height_ave = 0
    ...
    while STATE_THREAD:
        ...
        if not timeout:
            ...
            if ACTUAL_ALT > SONAR_FILTER_ALT:
                if math.fabs(height_offset - height_ave) > SONAR_MAX_CHANGE:
                    height_offset = height_ave

            # 均值濾波器(使線條滑順)
            height_ave = height_register.calculate(height_offset)

            # 卡爾曼濾波器
            ACTUAL_ALT_SONAR = height_register_kf.processing(height_ave)
        ...
        # 迴圈執行間隔
        while time.time()-t_loop < (1 / FREQ_SENSOR_SONAR):
            pass
        t_loop = time.time()
...
def speed_sensor():
    ...
    # 均值濾波器設定
    speed_register = MovingAverageFilter(SPD_SAMPLE_NUM_AVE)
    ...
    while STATE_THREAD:
        ...
        # 均值濾波處理
        speed = speed_register.calculate(speed_raw)
        ...
        # 迴圈執行間隔
        while time.time()-t_loop < (1 / FREQ_SENSOR_SPD):
            pass
        t_loop = time.time()
...
def command_transfer():
    ...
    while STATE_THREAD:
        ...
        # 迴圈執行間隔
        while time.time()-t_loop < (1 / FREQ_CMD):
            pass
        t_loop = time.time()
...
def laser_calibrate():
    ...
    for i in range(0, LASER_SAMPLE_NUM_CAL):
        # 量測迴圈
        height_raw = laser.get_distance() / 1000
...
def line_moment():
    ...
    for c in contours:
        ...
        if block_area >= ANG_LINE_TRACK_AREA_L:
            ...
            if h == 0 and img_th[y, x] == 255:
                ...
                line_angle = math.degrees(math.atan(w / h + 0.01))
                if img_th[y, x] == 255:
                    line_angle *= -1
    ...
    return block_x, block_y, area_sum, line_angle
...
def data_sheet():
    ...
    # 第四頁-循線控制器
    # 航向-滾轉控制器數值處理圖
    ds3.cell(1, 1, '航向-滾轉控制器數值處理圖')
    ds3.cell(2, 1, '與線中心距')
    for x in range(len(R_HDG_ROLL_ACTUAL_X)):
        ds3.cell(x + 3, 1, R_HDG_ROLL_ACTUAL_X[x])

    ds3.cell(2, 2, '輸出滾轉角')
    for x in range(len(R_HDG_ROLL_TARGET_ROLL)):
        ds3.cell(x + 3, 2, R_HDG_ROLL_TARGET_ROLL[x])

    ds3.cell(2, 3, '實際滾轉角')
    for x in range(len(R_HDG_ROLL_ACTUAL_ROLL)):
        ds3.cell(x + 3, 3, R_HDG_ROLL_ACTUAL_ROLL[x])

    # 航向-滾轉控制器PID數值處理圖
    ds3.cell(1, 5, '航向-滾轉控制器PID數值處理圖')
    ds3.cell(2, 5, 'Error')
    for x in range(len(R_HDG_ROLL_ERROR)):
        ds3.cell(x + 3, 5, R_HDG_ROLL_ERROR[x])

    ds3.cell(2, 6, 'Gain P')
    for x in range(len(R_HDG_ROLL_GP)):
        ds3.cell(x + 3, 6, R_HDG_ROLL_GP[x])

    ds3.cell(2, 7, 'Gain I')
    for x in range(len(R_HDG_ROLL_GI)):
        ds3.cell(x + 3, 7, R_HDG_ROLL_GI[x])

    ds3.cell(2, 8, 'Gain D')
    for x in range(len(R_HDG_ROLL_GP)):
        ds3.cell(x + 3, 8, R_HDG_ROLL_GP[x])

    # 航向-偏航控制器數值處理圖
    ds3.cell(1, 10, '航向-偏航控制器數值處理圖')
    ds3.cell(2, 10, '實際角度差')
    for x in range(len(R_HDG_YAW_ACTUAL)):
        ds3.cell(x + 3, 10, R_HDG_YAW_ACTUAL[x])

    ds3.cell(2, 11, '線角度')
    for x in range(len(R_HDG_YAW_ACTUAL_LINE)):
        ds3.cell(x + 3, 11, R_HDG_YAW_ACTUAL_LINE[x])

    ds3.cell(2, 12, '輸出偏航角速率')
    for x in range(len(R_HDG_YAW_TARGET_YAW_RATE)):
        ds3.cell(x + 3, 12, R_HDG_YAW_TARGET_YAW_RATE[x])

    # 航向控制器PID數值處理圖
    ds3.cell(1, 14, '航向-偏航控制器PID數值處理圖')
    ds3.cell(2, 14, 'Error')
    for x in range(len(R_HDG_YAW_ERROR)):
        ds3.cell(x + 3, 14, R_HDG_YAW_ERROR[x])

    ds3.cell(2, 15, 'Gain P')
    for x in range(len(R_HDG_YAW_GP)):
        ds3.cell(x + 3, 15, R_HDG_YAW_GP[x])

    ds3.cell(2, 16, 'Gain I')
    for x in range(len(R_HDG_YAW_GI)):
        ds3.cell(x + 3, 16, R_HDG_YAW_GI[x])

    ds3.cell(2, 17, 'Gain D')
    for x in range(len(R_HDG_YAW_GP)):
        ds3.cell(x + 3, 17, R_HDG_YAW_GP[x])

    # 飛行速度控制器
    ds3.cell(1, 19, '飛行速度控制器數值處理圖')
    ds3.cell(2, 19, '目標速度')
    for x in range(len(R_SPD_TARGET)):
        ds3.cell(x + 3, 19, R_SPD_TARGET[x])

    ds3.cell(2, 20, '實際速度')
    for x in range(len(R_SPD_ACTUAL)):
        ds3.cell(x + 3, 20, R_SPD_ACTUAL[x])

    ds3.cell(2, 21, '目標傾角')
    for x in range(len(R_SPD_TARGET_PITCH)):
        ds3.cell(x + 3, 21, R_SPD_TARGET_PITCH[x])

    ds3.cell(2, 22, '實際傾角')
    for x in range(len(R_SPD_ACTUAL_PITCH)):
        ds3.cell(x + 3, 22, R_SPD_ACTUAL_PITCH[x])

    # 飛行速度控制器PID數值處理圖
    ds3.cell(1, 24, '飛行速度控制器PID數值處理圖')
    ds3.cell(2, 24, 'Error')
    for x in range(len(R_SPD_ERROR)):
        ds3.cell(x + 3, 24, R_SPD_ERROR[x])

    ds3.cell(2, 25, 'Gain P')
    for x in range(len(R_SPD_GP)):
        ds3.cell(x + 3, 25, R_SPD_GP[x])

    ds3.cell(2, 26, 'Gain I')
    for x in range(len(R_SPD_GI)):
        ds3.cell(x + 3, 26, R_SPD_GI[x])

    ds3.cell(2, 27, 'Gain D')
    for x in range(len(R_SPD_GP)):
        ds3.cell(x + 3, 27, R_SPD_GP[x])

    # 飛行速度控制器
    ds3.cell(1, 29, '飛行速度控制器數值處理圖-開迴路')
    ds3.cell(2, 29, '目標俯仰角')
    for x in range(len(R_SPD_OL_TARGET_PITCH)):
        ds3.cell(x + 3, 29, R_SPD_OL_TARGET_PITCH[x])

    ds3.cell(2, 30, '實際俯仰角')
    for x in range(len(R_SPD_OL_ACTUAL_PITCH)):
        ds3.cell(x + 3, 30, R_SPD_OL_ACTUAL_PITCH[x])

    ds3.cell(2, 31, '實際速度')
    for x in range(len(R_SPD_OL_ACTUAL_SPD)):
        ds3.cell(x + 3, 31, R_SPD_OL_ACTUAL_SPD[x])
    ...
    # 雷射測距儀數值處理圖
    ds4.cell(1, 16, '雷射測距儀數值處理圖')
    ds4.cell(2, 16, '原始值')
    for x in range(len(R_ACTUAL_ALT_LASER_RAW)):
        ds4.cell(x + 3, 16, R_ACTUAL_ALT_LASER_RAW[x])

    ds4.cell(2, 17, '補正後')
    for x in range(len(R_ACTUAL_ALT_LASER_OFFSET)):
        ds4.cell(x + 3, 17, R_ACTUAL_ALT_LASER_OFFSET[x])

    ds4.cell(2, 18, '均值濾波')
    for x in range(len(R_ACTUAL_ALT_LASER)):
        ds4.cell(x + 3, 18, R_ACTUAL_ALT_LASER[x])
...
def alt_sen_data():
    ...
    # 雷射測距儀數值處理圖
    num_alt = []
    for i in range(1, len(R_ACTUAL_ALT_LASER_RAW) + 1):
        num_alt.append(i)

    plt.plot(num_alt, R_ACTUAL_ALT_LASER_RAW, label="Raw Data")
    plt.plot(num_alt, R_ACTUAL_ALT_LASER_OFFSET, label="Angle & Height Offset")
    plt.plot(num_alt, R_ACTUAL_ALT_LASER, label="Average Filter")

    plt.xlabel('Data number')
    plt.ylabel('Height(m)')
    plt.title('Altitude Data - Laser')
    plt.legend()
    plt.savefig('Altitude Data - Laser.png')
    plt.show()
...
def hdg_ctrl_data():
    # 航向控制器響應圖
    num_hdg = []
    for i in range(1, len(R_HDG_ROLL_ACTUAL_X) + 1):
        num_hdg.append(i)

    # 航向-滾轉控制器響應圖
    plt.plot(num_hdg, R_HDG_ROLL_ACTUAL_X, label="Line Distance")
    plt.plot(num_hdg, R_HDG_ROLL_ACTUAL_ROLL, label="Actual Roll")
    plt.plot(num_hdg, R_HDG_ROLL_TARGET_ROLL, label="Target Roll")

    plt.xlabel('Data number')
    plt.ylabel('Distance(m), Angle(degree)')
    plt.title('Heading Controller Roll Data')
    plt.legend()
    plt.savefig('Heading Controller Roll Data.png')
    plt.show()

    # 航向-滾轉控制器PID響應圖
    plt.plot(num_hdg, R_HDG_ROLL_ERROR, label="Error")
    plt.plot(num_hdg, R_HDG_ROLL_GP, label="Gain P")
    plt.plot(num_hdg, R_HDG_ROLL_GI, label="Gain I")
    plt.plot(num_hdg, R_HDG_ROLL_GD, label="Gain D")

    plt.xlabel('Data number')
    plt.ylabel('Angle(degree), Angle Rate(degree/s)')
    plt.title('Heading Controller Roll PID Output Data')
    plt.legend()
    plt.savefig('Heading Controller Roll PID Output Data.png')
    plt.show()

    # 航向-偏航控制器響應圖
    plt.plot(num_hdg, R_HDG_YAW_ACTUAL, label="Actual Heading")
    plt.plot(num_hdg, R_HDG_YAW_ACTUAL_LINE, label="Line Angle")
...
def take_off(target_alt=1.0, color=''):
    ...
    while ACTUAL_ALT < TARGET_ALT * (TAKEOFF_SETTLE / 100):
        ...
        if i % 5 == 0:
            ...
            print('<高度感測器狀態> 超音波：%.2f m，IMU：%.2f m，雷射：%.2f m' %
                  (ACTUAL_ALT_SONAR, ACTUAL_ALT_IMU, ACTUAL_ALT_LASER))
