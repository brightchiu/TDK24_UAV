# 0902_move_control 前進控制(不含左右方向)
# 包含高度感測器校正、羅盤補正 前進及定位指令
# 針對定位值使用均值處理
...
TARGET_ALT = 0
TARGET_THRUST = 0
TARGET_ROLL_RATE = 0             # No function
TARGET_PITCH_RATE = 0            # No function
TARGET_YAW_RATE = 0              # No function
...
HDG_DZ = 2           # Heading dead zone (oneway, unit:degree)
...
BODY_REF_X = int(IMAGE_WEIGHT / 2)         # 機身位置X座標
BODY_REF_Y = int(IMAGE_HEIGHT / 2)         # 機身位置Y座標
FPS = 10                                  # 畫面刷新延遲時間
VIDEO = cv2.VideoCapture(0)
BLOCK_AREA_LIMIT = 6000                   # 色塊大小下限值
BLOCK_COLOR = 'red'                       # Target color
MISSION_COLOR = 'green'                   # 比賽顏色
...
def position_controller():
    ...
    while STATE:
        while MODE == 'POS':
            # Camera Section
            x_row, y_row, area_row = img_cal(BLOCK_COLOR)

            # Filter Section
            x_register.insert(0, x_row)
            if len(x_register) > SAMPLE_NUMBER:
                x_register.pop()
            x = np.average(x_register)

            y_register.insert(0, y_row)
            if len(y_register) > SAMPLE_NUMBER:
                y_register.pop()
            y = np.average(y_register)
            ...
            elif y < -POS_DZ:  # Backward
                TARGET_PITCH = -POS_ANGLE_LIMIT
            else:  # In dead zone
                TARGET_PITCH = 0

            # Record data
            R_ACTUAL_POS_X.append(x)
            R_ACTUAL_POS_Y.append(y)
...
def heading_controller():
    ...
    while STATE:
        while MODE == 'LINE':
            ...
            # Judge Section
            if ACTUAL_HDG > HDG_DZ:          # Right
                TARGET_YAW = TARGET_YAW + ACTUAL_HDG
            elif ACTUAL_HDG < -HDG_DZ:       # Left
                TARGET_YAW = TARGET_YAW + ACTUAL_HDG
            else:  # In dead zone
...
def img_cal(color):
    ...
    for c in contours:
        ...
        if area >= BLOCK_AREA_LIMIT:
            ...
            x = cx - BODY_REF_X  # 色塊座標-畫面中央x座標 (右正左負)
            y = BODY_REF_Y - cy  # 畫面中央y座標-色塊座標 (上正下負)
            print("相對座標為:%4.1f,%4.1f" % (x, y))

            return x, y, area


# Wait For Detect Color
def color_detect(color):
    while 1:
        x, y, area = img_cal(color)
        if area > BLOCK_AREA_LIMIT:
            break
        time.sleep(1/FREQ_COLOR)
...
# Move forward CMD
def flight_forward():
    global MODE, TARGET_PITCH, TARGET_ROLL
    MODE = 'LINE'
    print('Change to Line Mode (Move forward).')
    TARGET_PITCH = 5
    TARGET_ROLL = 0
...
def chg_to_pos():
    global MODE, TARGET_PITCH, TARGET_ROLL, TARGET_YAW
...
def unload_package():
    # Set the pin mode
    GPIO.setmode(GPIO.BCM)
    GPIO.setup(SV_PIN, GPIO.OUT)

    # Creat PWM control item
    servo_r = GPIO.PWM(SV_PIN, SV_PWM_FREQ)
...
def take_off(target_height):
    ...
    # Altitude check
    while ACTUAL_ALT < target_height:
