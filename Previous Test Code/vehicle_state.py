...
print
" New Home Location (from attribute - altitude should be 222): %s" % vehicle.home_location
...
cmds.wait_ready()
print
" New Home Location (from vehicle - altitude should be 222): %s" % vehicle.home_location
...
def attitude_callback(self, attr_name, value):
    ...
    if value != last_attitude_cache:
        print
        " CALLBACK: Attitude changed to", value
...
def decorated_mode_callback(self, attr_name, value):
    ...
    # `value` is the updated attribute value.
    print
    " CALLBACK: Mode changed to", value


print
" Set mode=STABILIZE (currently: %s) and wait for callback" % vehicle.mode.name
...
print
" Read new value of param 'THR_MIN': %s" % vehicle.parameters['THR_MIN']
...
def decorated_thr_min_callback(self, attr_name, value):
    print
    " PARAMETER CALLBACK: %s changed to: %s" % (attr_name, value)
...
def any_parameter_callback(self, attr_name, value):
    print
    " ANY PARAMETER CALLBACK: %s changed to: %s" % (attr_name, value)
